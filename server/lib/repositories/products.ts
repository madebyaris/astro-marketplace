...
export async function listProducts(db: DatabaseAdapter): Promise<Product[]> {
...
export async function createProduct(db: DatabaseAdapter, input: Omit<Product, 'id'>): Promise<Product> {
...
export async function updateProduct(db: DatabaseAdapter, id: number, input: Partial<Product>): Promise<Product | null> {
...
export async function deleteProduct(db: DatabaseAdapter, id: number): Promise<boolean> {
  await execute(db, `DELETE FROM affiliate_links WHERE product_id = ?`, [id]);
